...
if len(sys.argv) < 3:
    print(
        "Please run with at least two parameters: your_name, position_number, start_gesture_number[opt].")
    exit(0)
if len(sys.argv) > 4:
...
if len(sys.argv) < 4:
    ...
    single_diff_vec_list = convert_batch_list_to_posture_list(
        single_raw_vec_list)
    single_diff_vec_array = np.array(single_diff_vec_list)
    np.save(volunteer+"_"+position+"_"+"single.npy", single_diff_vec_array)

multiple_null_vec_array, multiple_finger_vec_array = multiple_finger_collection(
    ser, batch, start_gesture_number)
